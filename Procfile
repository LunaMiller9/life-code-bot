...
PAID_CALCULATOR_URL = "https://tvoy-life-code.netlify.app"  # ссылка на платный калькулятор (с днём цикла)
IMAGES_DIR = "images"                       # папка с number_1.png ... number_9.png
# ========================================================
...
async def send_number_card(chat_id: int, life_number: int):
    ...
    caption = (
        f"Твоя Цифра Я = <b>{life_number}</b>\n\n"
        "Это программа, которая формирует твою жизнь — с какого дня начинается твой персональный цикл, "
        "какая энергия ведёт тебя и куда закономерно движется ситуация.\n\n"
        "Хочешь пойти глубже — узнать, что делать в каждый из 9 дней твоего цикла?\n"
        f"Полный разбор здесь: {PAID_CALCULATOR_URL}\n\n"
        "Хочешь узнать цифру близкого человека — просто напиши /start ещё раз."
    )
